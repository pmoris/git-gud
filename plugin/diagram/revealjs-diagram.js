(function (window) {
  ...
  function addDouble(name, parent1, parent2, root) {
    if (!parent1 || !parent2 ) {
      throw Error('You need to specify a parent to add a node in the Tree!')
    }
    return function() {
      var parentNode1 = findNode(parent1, [root]);
      if (!parentNode1.children) {
        parentNode1.children = []
      }
      parentNode1.children.push({
        name: name
      })
      update(root);

      var parentNode2 = findNode(parent2, [root]);
      if (!parentNode2.children) {
        parentNode2.children = []
      }
      parentNode2.children.push({
        name: name
      })
      update(root);
...
function connectNodes(node1, node2, root) {
  return function() {
    var node = findNode(node1, [root]);
    node.children.push({
      name: node2
    })
    update(root)
